...
class chatting_page : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        messagesReference = database.getReference("Chatting")
        // Inside your onCreate method or wherever appropriate

        val receiverNameRef = FirebaseDatabase.getInstance().getReference("Registered users").child(receiverUID).child("Name")
        ...
        val senderNameRef = FirebaseDatabase.getInstance().getReference("Registered users").child(senderuid).child("Name")
    ...
    private fun sendMessage() {
        ...
        FirebaseDatabase.getInstance().getReference("Chatting").addListenerForSingleValueEvent(object : ValueEventListener {
...
var x=""
                    for (chatSnapshot in snapshot.children) {
                        ...
                        //    var allmsgs= mapOf(
                          ...
                          //  )
                           ...
                           // messagesReference.child(x).child("messages").child(bal!!).setValue(messageText)
            ...
            // Create a unique key for the message
                //     val messageKey = messagesReference.child(chatRoomId).push().key


            // Set the message under the specific chat room
            //messagesReference.child(chatRoomId).child("messages").child(messageKey!!).setValue(messageText)
...
/*
    private fun fetchPreviousMessages() {
        Log.d("Bal", "das")
        Toast.makeText(this, "adsfsdf", Toast.LENGTH_SHORT).show()
            ...
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    for (chatSnapshot in dataSnapshot.children) {
                        ...
                        // Traverse through all message keys inside the chat room ID
                        for (messageSnapshot in chatSnapshot.children) {
                            val messageId = messageSnapshot.key

                            // Assuming the structure is messages -> chatRoomId -> messageId -> SenderUID
                            val senderUIDSnapshot = messageSnapshot.child("Sender UID")
                            val receiverUIDSnapshot = messageSnapshot.child("Receiver UID")

                            if (senderUIDSnapshot.exists() && receiverUIDSnapshot.exists()) {
                                var gotsender = senderUIDSnapshot.getValue(String::class.java).toString()
                                var gotreceiver = receiverUIDSnapshot.value.toString()
                                if ((gotsender == senderuid && gotreceiver == receiverUID)

                                ) {
                                    var chatMessage =
                                        messageSnapshot.child("chatMessage").getValue(ChatMessage::class.java)
                                    chatMessage?.let { messagesList.add(it) }
                                }


                                else if ((gotsender == receiverUID && gotreceiver == senderuid)){
                                    receiverUID = senderuid.also { senderuid = receiverUID }
                                }
                            }
                        }
                    }

                    // Sort the messages list by timestamp
                    messagesList.sortBy { it.timestamp }

                    // Display the sorted messages
                    for (message in messagesList) {
                        val senderName = if (message.senderId == senderuid) senderName else receiverName
